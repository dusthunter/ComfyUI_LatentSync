...
class LipsyncPipeline(DiffusionPipeline):
    ...
    def __call__(
        self,
        # Changed inputs: Directly accept tensors
        video_frames_uint8_np: np.ndarray, # Input video frames as uint8 numpy array (F x H x W x C)
        audio_waveform_float_tensor: torch.Tensor, # Input audio waveform tensor (Channels x Samples)
        ...
        **kwargs,
        ...
        # Derive height and width from input frames if not provided
        if height is None or width is None:
            if video_frames_uint8_np.ndim == 4:
                _, height, width, _ = video_frames_uint8_np.shape
            else:
                 raise ValueError("Input video_frames_uint8_np must be a 4D array (F x H x W x C)")
        ...
        whisper_feature = self.audio_encoder.audio2feat4tensor(audio_waveform_float_tensor)
        ...
        # Output IMAGE: Tensor, T x H x W x C, float 0-1, on CPU
        # synced_video_frames_uint8_np is F x H x W x C, uint8 numpy
        output_frames_tensor = torch.from_numpy(synced_video_frames).float() / 255.0
        output_frames_tensor = output_frames_tensor.cpu() # Ensure on CPU
        print("Inference complete. Returning tensors.")
        return (output_frames_tensor, )
