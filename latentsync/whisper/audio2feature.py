...
class Audio2Feature:
    ...
    def audio2feat4tensor(self, audio_waveform_tensor: torch.Tensor):
        # Ensure the tensor is single channel and on the correct device
        if audio_waveform_tensor.ndim == 2:
             if audio_waveform_tensor.shape[0] > 1:
                  audio_input = audio_waveform_tensor[0, :]
             else:
                  # Already single channel [1, Samples], remove the channel dimension
                  audio_input = audio_waveform_tensor.squeeze(0)
        elif audio_waveform_tensor.ndim == 1:
             # Already single channel [Samples]
             audio_input = audio_waveform_tensor
        else:
             raise ValueError(f"Unsupported tensor shape: {audio_waveform_tensor.shape}. Expected [Samples] or [Channels, Samples].")

        # Ensure tensor is on the same device as the model
        audio_input = audio_input.to(self.model.device)
